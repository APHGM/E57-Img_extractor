...
def extract_point_cloud_to_laz(e57_path, output_folder=None):
    """
    Extracts point cloud data from E57 file and saves as LAZ format
    ...
    try:
        ...
        print("Error: laspy and pdal packages required for LAZ export")
        print("Install with: pip install laspy pdal")
        return False
    ...
    print(f"Extracting point cloud to {laz_path}...")
    
    try:
        # Create PDAL pipeline
        pipeline = pdal.Pipeline()
        
        # Read E57 and write LAZ
        pipeline |= pdal.Reader.e57(filename=e57_path)
        pipeline |= pdal.Writer.las(filename=laz_path, compression=True)
        
        # Execute the pipeline
        pipeline.execute()
        
        print(f"Successfully saved point cloud to {laz_path}")
        return True
...
class E57ImageExtractorApp:
    def __init__(self, root):
        ...
        # Add point cloud extraction checkbox
        self.extract_point_cloud_var = tk.BooleanVar(value=False)
        # self.extract_point_cloud_var(side=tk.LEFT, padx=5)

        self.pc_checkbox = ttk.Checkbutton(
            self.output_frame,
            text="Extract Point Cloud (LAZ)",
    ...
    def start_extraction(self):
        ...
        def extraction_thread():
            try:
                # Extract images (original functionality)
                extract_images_from_e57(input_file, output_folder)
                
                # Extract point cloud if requested
                if extract_pc:
                    success = extract_point_cloud_to_laz(input_file, output_folder)
                    if not success:
                ...
                self.root.after(0, lambda: self.browse_button.config(state=tk.NORMAL))
                self.root.after(0, lambda: self.output_browse_button.config(state=tk.NORMAL))
                self.root.after(0, lambda: self.pc_checkbox.config(state=tk.NORMAL))
                self.root.after(0, lambda: self.status_var.set("Extraction complete"))
